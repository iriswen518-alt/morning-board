...
5. **嚴禁沿用/複製前幾日的國際新聞充數**：每則的 `(YYYY-MM-DD)` 日期必須落在近 24~48 小時內；若最近可確認外電是上一交易日（如週一早上僅有上週五收盤），該則標題須明標「（…，最近交易日）」且僅限「美股收盤回顧」一則，其餘各則一律當次重新 WebSearch。週末／連假後（週六、日、週一）務必額外搜當日亞股／歐股開盤、油價、地緣時事，避免整區凍結在上一交易日。抓不到就走第 4 點的「暫無」單行，**絕不可把舊項原封搬過來**。
6. 此區摘要：中文 60~120 字、英文 40~80 words，只依實際抓到的內容改寫，嚴禁杜撰數字/引述。
