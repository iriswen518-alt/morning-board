...
def parse_one_item(blk: str) -> Optional[dict]:
    ...
    # Extract publication date from title bracket, e.g. （2026-07-06）or (2026-07-06)
    published = ""
    if title_zh:
        dm = re.search(r"[（(](\d{4}-\d{2}-\d{2})[）)]", title_zh)
        if dm:
            published = dm.group(1)
    # Also check title_en if nothing found in zh title
    if not published and title_en:
        dm = re.search(r"[（(](\d{4}-\d{2}-\d{2})[）)]", title_en)
        if dm:
            published = dm.group(1)
